/*********BLE_DHT11 *******
 *用手机蓝牙读取DHT11传感器数据实验
 两部分：是DHT11和gatt_server工程的综合使用
 实现功能：手机APP能读到传感器数据变化
***********************/
/***********dht11头文件***/
//#include <stdio.h>
#include "esp_log.h"
...
/* One gatt-based profile one app_id and one gatts_if, （一个gatt-based的profile,由一个app_id和一个gatts_if组成）
this array will store the gatts_if returned by ESP_GATTS_REG_EVT （此数组将存储ESP_gatts_REG_EVT返回的gatts_if）*/
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ESP_LOGI(GATTS_TAG, "GATT_READ_EVT, conn_id %d, trans_id %d, handle %d\n,温度：%d,湿度：%d", param->read.conn_id, param->read.trans_id, param->read.handle,DHT11_read().temperature,DHT11_read().humidity);
        esp_gatt_rsp_t rsp;
        memset(&rsp, 0, sizeof(esp_gatt_rsp_t));
        rsp.attr_value.handle = param->read.handle;
        rsp.attr_value.len = 4;
        rsp.attr_value.value[0] = 0xde;//DHT11_read().temperature;
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        rsp.attr_value.handle = param->read.handle;
        rsp.attr_value.len = 4;
        rsp.attr_value.value[0] = 0xde;//0xde
        rsp.attr_value.value[1] = 0xed;
        rsp.attr_value.value[2] = 0xbe;
        rsp.attr_value.value[3] = 0xef;
        esp_ble_gatts_send_response(gatts_if, param->read.conn_id, param->read.trans_id,
...
void app_main()
{
   /**dht11部分**/
    ...
    /**蓝牙部分**************************************************/
   esp_err_t ret;  //   定义出处 typedef int32_t esp_err_t; 

    // Initialize NVS. NVS初始化   非易失存储器初始化
    ret = nvs_flash_init();  //
    ...
    ESP_ERROR_CHECK(esp_bt_controller_mem_release(ESP_BT_MODE_CLASSIC_BT));//释放经典蓝牙资源，保证设备不工作在经典蓝牙下面

    esp_bt_controller_config_t bt_cfg = BT_CONTROLLER_INIT_CONFIG_DEFAULT();//初始化蓝牙控制器，此函数只能被调用一次，且必须在其他蓝牙功能被调用之前调用
...
//注册GATT回调函数,回调内容后面细说
    ...
    */
     ...
     * 最后，使用应用程序ID注册应用程序配置文件，该ID是用户分配的编号，用于标识每个配置文件。这样，多个应用程序配置文件可以在一台服务器上运行。**/
